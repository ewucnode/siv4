...
const scenarios: JournalScenario[] = [
  ...
  {
    id: 'stock-adjustment',
    title: 'Stock Adjustment / Reduction (Quantity Correction)',
    category: 'Inventory',
    icon: ArrowLeftRight,
    iconColor: 'bg-teal-50 text-teal-600',
    summary: 'Manual stock increases and decreases are quantity corrections, so they post against Opening Balance Equity — never the P&L. Adjusting stock never changes your profit.',
    trigger: 'Inventory page stock edit / CSV import → create_opening_batch (adjustment) & create_stock_reduction RPCs',
    entries: [
      {
        description: 'Stock increase (quantity correction)',
        debitAccount: 'Inventory Asset',
        debitCode: '1200',
        creditAccount: 'Opening Balance Equity',
        creditCode: '3900',
        amount: 'Qty × Unit Cost',
        explanation: 'Inventory goes up (debit) and the offset goes to equity, not income — a quantity correction is not a sale or a gain.',
      },
      {
        description: 'Stock decrease (quantity correction)',
        debitAccount: 'Opening Balance Equity',
        debitCode: '3900',
        creditAccount: 'Inventory Asset',
        creditCode: '1200',
        amount: 'FIFO layer value consumed (+ shortfall at cost)',
        explanation: 'Inventory goes down (credit) and the offset comes from equity, not expense — correcting a mistaken quantity is not a loss.',
      },
    ],
    example: {
      context: 'Fix a miscounted cable SKU: increase 2 units @ 10, then reduce 1 unit @ 10 (one layer)',
      lines: [
        { account: 'Inventory Asset (1200)', code: '1200', debit: '20', credit: '—' },
        { account: 'Opening Balance Equity (3900)', code: '3900', debit: '—', credit: '20' },
        { account: 'Opening Balance Equity (3900)', code: '3900', debit: '10', credit: '—' },
        { account: 'Inventory Asset (1200)', code: '1200', debit: '—', credit: '10' },
      ],
    },
    notes: [
      'Owner decision 2026-09-07: stock adjustments in this business are quantity corrections (mistaken increases/decreases), so they belong in equity, not the P&L. The Aug 28 - Sep 6 cleanup batch was reclassified to 3900 per-date.',
      'Account 5900 (Inventory Adjustment) is a legacy account with no writers since 2026-09-07 — it stays in the chart for history.',
      'Genuine damage or theft is a real loss: book it as a manual journal entry to an expense account (Journal page → New Entry), not via a stock adjustment.',
      'The negative-layer purge on the Inventory Audit page also posts to 3900.',
    ],
  },
  {
    ...
    entries: [
      {
        ...
        explanation: 'Inventory is recognized as an asset (debit), and the corresponding capital is recorded as Opening Balance Equity (credit) — the same account that absorbs stock quantity corrections. This balances the books from day one.',
      },
      {
        ...
        explanation: 'Cash on hand is an asset (debit), funded by owner capital in Opening Balance Equity (credit).',
      },
    ],
    notes: [
      'Opening balances are posted once during system setup.',
      'Opening stock is posted by the create_opening_batch RPC as Dr 1200 / Cr 3900 — the same routing stock quantity corrections use.',
