-- Offline invoice printing: persist the device's temporary number so a
-- receipt printed while offline stays findable after sync assigns the real
-- POS-/INV- number. The sales-list search and the invoice-view "offline ref"
-- chip read this column; queued (unsynced) rows already show the temp number
-- as their invoice_number via the pending overlay.

alter table invoices
  add column if not exists client_temp_number text;

create index if not exists idx_invoices_client_temp_number
  on invoices (client_temp_number)
  where client_temp_number is not null;

-- sync_invoice_create: store the outbox payload's temp_number.
...
begin
  ...
  if p_payload ? 'id' then
    insert into invoices (id, invoice_number, customer_id, invoice_date, due_date, subtotal, discount_amount,
      ...
      status, is_pos, reference, notes, idempotency_key, client_temp_number)
