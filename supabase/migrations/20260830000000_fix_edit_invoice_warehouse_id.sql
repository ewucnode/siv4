/*
# Fix: edit_invoice warehouse-aware stock restoration

## Problem
edit_invoice STEP 1b always restores stock to the default warehouse (v_default_wh),
regardless of which warehouse the original sale deducted from. STEP 6 also doesn't
pass warehouse_id when re-inserting items, so the deduction trigger falls back to
the default warehouse. Net effect: stock silently migrates to the default warehouse
on every invoice edit.

## Fix
1. STEP 1b: Use v_item.warehouse_id (fall back to v_default_wh if NULL)
2. STEP 6: Include warehouse_id in the INSERT for re-inserted items
*/
...
BEGIN
  ...
  -- STEP 1b: Restore stock for old items — use each item's original warehouse
  FOR v_item IN SELECT * FROM invoice_items WHERE invoice_id = p_invoice_id LOOP
    ...
    IF v_target_wh IS NOT NULL THEN
      ...
      -- Clean up the old sale movement for this item so the AFTER INSERT trigger
      -- fires fresh on re-insert in STEP 6 without stale state
  ...
  -- STEP 6: Re-insert items — include warehouse_id so the deduction trigger
  -- deducts from the correct warehouse
