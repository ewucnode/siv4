-- Rewrite get_cogs_audit() from row-by-row PL/pgSQL (~10 queries per
-- invoice, degrading linearly with invoice count) to a single set-based
-- SQL statement with a fixed number of scans/aggregates.
--
-- Motivation: the loop version hit 8.9s at 580 invoices — over the 8s
-- statement_timeout of the `authenticated` role, failing the COGS Audit
-- page ("canceling statement due to statement timeout"). Even gated
-- (20260831170000), it remains O(invoices) in query count.
--
-- The output contract matches 20260831170000 (same columns, same status /
-- keeper / orphan logic; verified row-by-row against it on the live DB).
-- Two intentional, financially-neutral differences where the old function
-- was nondeterministic:
--   * keeper ties (same |total - expected| and entry_date) now break by
--     lowest JE id — previously arbitrary planner order
--   * FIFO batch_seq ties (batches with identical created_at) now break by
--     consumption id — previously arbitrary
-- all_je_diff stays always 0 — the loop version declared it but never
-- assigned it.
--
-- Also note: the first column remains aud_invoice_id on purpose; the
-- audit page maps it to invoice_id client-side.
...
SELECT
  ...
  ROUND(COALESCE(c.journal_cogs, 0), 2)                                    AS journal_cogs_c,
  c.je_cnt                                                                 AS journal_je_count,
  ROUND(COALESCE(c.fifo_cogs, 0), 2)                                       AS fifo_cogs_d,
...
5000 of postings + reversals). Fixed-cost query — one pass regardless of invoice count.';
